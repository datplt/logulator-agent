...
io.forceDestroy = function() {
  debuglog(' + listening the exit/SIGINT event');
  logContext.socket && logContext.socket.disconnect();
  return logGenerator.destroy().finally(function() {
    debuglog(' - the generator has been destroyed');
    io.close();
    return {};
  });
}
