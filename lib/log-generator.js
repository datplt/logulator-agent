...
LogGenerator.prototype.initWorker = function(metainf) {
    ...
    return self.__validateWorker(metainf).then(function(meta) {
        ...
        var copiedFields = [ 'delayTime', 'delayOffset', 'template', 'logtarget' ];
        ...
        debuglog(' - delayTime: %s+%s', worker.delayTime, worker.delayOffset);
        ...
        if (lodash.isEmpty(worker.timer)) {
          ...
          }, worker.delayTime, worker.delayOffset);
        } else {
...
LogGenerator.prototype.updateFrequency = function(id, delayTime, delayOffset) {
    var self = this;
    return self.__retrieveWorker(id).then(function(worker) {
      var oldDelayTime = worker.delayTime;
      if (delayTime) {
        worker.delayTime = delayTime;
      }

      var oldDelayOffset = worker.delayOffset;
      if (delayOffset) {
        worker.delayOffset = delayOffset;
      }

      worker.timer.reset(worker.delayTime, worker.delayOffset);

      return Promise.resolve({
        workerId: id,
        code: 200,
        message: "worker has reset successfully",
        old_delay_segment: oldDelayTime,
        old_delay_offset: oldDelayOffset,
        new_delay_segment: delayTime,
        new_delay_offset: delayOffset
      });
    });
}
