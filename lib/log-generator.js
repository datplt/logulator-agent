...
var TargetLogfile = require('./target-logfile.js');
var TargetRestapi = require('./target-restapi.js');
var TargetMongodb = require('./target-mongodb.js');
...
LogGenerator.prototype.initWorker = function (metainf) {
    ...
    return self.__validateWorker(metainf).then(function (meta) {
        ...
        switch (meta.target.type) {
            case "logfile":
                worker.target = new TargetLogfile(meta.target);
                break;
            case "restapi":
                worker.target = new TargetRestapi(meta.target);
                break;
            case "mongodb":
                worker.target = new TargetMongodb(meta.target);
                break;
            default:
                debuglog(' - unknown target type: %s', meta.target.type);
        }
...
LogGenerator.prototype.dropWorker = function (id) {
    var self = this;
    return self.stopWorker(id).then(function (info) {
        if (!(self._workers[id].target && self._workers[id].target.destroy)) {
            return info;
        }
        return self._workers[id].target.destroy();
    }).then(function () {
...
LogGenerator.prototype.destroy = function () {
    debuglog(" + destroy the Generator engine");
    return this.dropWorkers();
}
        ...
        "target": {
            ...
            "oneOf": [
                ...
                },
                {
                    ...
                    "required": ["type", "connectionString", "collectionName"]
                }
            ]
        }
    },
