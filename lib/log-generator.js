...
var OutputLogfile = require('./output-logfile.js');
...
LogGenerator.prototype.initWorker = function(metainf) {
    ...
    return self.__validateWorker(metainf).then(function(meta) {
        ...
        debuglog(' - logtarget: %s', JSON.stringify(worker.logtarget));
        if (lodash.isObject(worker.output)) {
          debuglog(' - worker.output has already existed');
          worker.output.destroy && worker.output.destroy();
        }
        worker.output = new OutputLogfile(worker.logtarget);

        debuglog(' = worker: %s', JSON.stringify(lodash.omit(worker, ['timer', 'output'])));
...
LogGenerator.prototype.__writeLog = function(id, data) {
    ...
    worker.output.push(msg, function(err) {
        if (err) {
